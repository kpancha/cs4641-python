...
class NaiveBayes(object):
    ...
    def _likelihood_ratio(self, XH, XM):  # [5pts]
        ...
        nH, D = XH.shape
        total_H = np.sum(XH, axis=0) + 1
        prob_H = total_H / np.sum(total_H)
            
        # Estimate probability of each word in vocabulary being used by Madison
        
        nM, D = XM.shape
        total_M = np.sum(XM, axis=0) + 1
        prob_M = total_M / np.sum(total_M)
        
        # Compute ratio of these probabilities
        
        fratio = prob_H / prob_M
        return fratio
    ...
    def _priors_ratio(self, XH, XM):  # [5pts]
        ...
        '''
        total_H_words = np.sum(np.sum(XH, axis=1))
        total_M_words = np.sum(np.sum(XM, axis=1))
        return total_H_words / total_M_words
    ...
    def classify_disputed(self, fratio, pratio, XD):  # [5pts]
        ...
        '''
        print(fratio)
        fratio = np.prod(np.power(fratio, XD), axis=1)
        print(fratio)
        prob = pratio * fratio
        print(prob)
        results = []
